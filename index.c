...
int index_add(Index *index, const char *path) {
    ...
    void *contents = malloc(fsize > 0 ? fsize : 1);
    if (!contents) { fclose(f); return -1; }
    if (fsize > 0) {
        size_t got = fread(contents, 1, fsize, f);
        if (got != fsize) { free(contents); fclose(f); return -1; }
    }
    fclose(f);
    ObjectID blob_id;
    if (object_write(OBJ_BLOB, contents, fsize, &blob_id) != 0) {
        free(contents);
        return -1;
    }
    free(contents);
